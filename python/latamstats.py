...
# Config FH2 (BD local separada de PR; upload a api/upload_fh2.php)
STATS_DB_PATH = 'C:/fh2_db/stats.sqlite3'
STATS_UPLOAD_URL = 'https://stats.latamsquad.org/api/upload_fh2.php'
STATS_API_KEY = 'xYUvckvpeeUW0MLTwAFcKZeyi5SYGBv8'
# Instancia fh2_1 = servidor FH2 #1 (tabla local stats1). Distinto de PR (pr-N).
STATS_SERVER_ID = 'fh2-1'
STATS_UPLOAD_TIMEOUT = 8
STATS_UPLOAD_LOG = 'C:/fh2_db/latamstats_upload.log'
# Cooldown de !stats por jugador (segundos).
STATS_CMD_COOLDOWN_SEC = 10
# Base GeoIP: IP2Location DB1 (.bin). Fallback CSV opcional.
STATS_GEOIP_PATH = 'C:/fh2_db/geo_ip.bin'

# Tabla SQLite por servidor (stats.sqlite3 de FH2, una tabla por instancia).
STATS_SERVER_TABLES = {
    'fh2-1': 'stats1',
    'fh2-2': 'stats2',
    'fh2-3': 'stats3',
...
# ------------------------------------------------------------------
DEFAULT_GEOIP_CSV = 'C:/fh2_db/GeoIPCountryWhois.csv'
...
def _upload_via_curl(body):
    """Fallback Windows: curl.exe (mejor HTTPS/TLS que el Python embebido de BF2)."""
    tmp_path = None
    try:
...
def strip_chat_hud_prefix(msg_text):
    """Quita prefijos HUD del motor BF2/FH2 antes de parsear comandos."""
    if msg_text is None:
        return ''
...
# Imports del motor BF2/FH2: protegidos para que tests offline importen sin bf2
try:
    import bf2
    import host
    _IN_GAME = True
except Exception:
    # ImportError o SyntaxError del stub Py2 fuera del servidor
    bf2 = None
    host = None
    _IN_GAME = False
...
def _debug_log(message):
    """Log de debug en juego (print seguro si no hay logger FH2)."""
    try:
        print('latamstats: %s' % message)
    except Exception:
        pass
...
def get_connected_players():
    """Lista jugadores conectados (API FH2/BF2)."""
    if bf2 is None:
        return []
    try:
        return list(bf2.playerManager.getPlayers())
    except Exception:
        return []


def init():
    """Registra el sistema de estadisticas al iniciar el mod."""
    if _IN_GAME:
        StatsSystem()
...
class StatsSystem(object):
    ...
    def __init__(self):
        # Asegura carpeta de BD antes de abrir SQLite.
        try:
            db_dir = os.path.dirname(STATS_DB_PATH)
    ...
    def _merge_round_snapshots(memory_snapshots, endgame_snapshots):
        """Une snapshots; el valor leido en EndGame tiene prioridad."""
    ...
    @staticmethod
    def _pm(player, msg):
        """Mensaje privado al jugador (canal Player / 14 en BF2)."""
        if player is None or host is None:
            return
        try:
        ...
        try:
            ...
            if len(text) > 240:
                text = text[:239]
            # 14 = canal privado tipico BF2 (mismo valor que usa PR).
    ...
    def _player_country(player):
        """Codigo ISO-2 via GeoIP local, o ''."""
    ...
    def on_player_disconnect(self, player):
        """Conserva el ultimo estado del jugador sin escribir aun en SQLite."""
    ...
    def _flush_round_and_upload(self):
        ...
        try:
            endgame_snapshots = {}
            for player in get_connected_players():
                snapshot = self._snapshot_player(player)
                if snapshot is not None:
            ...
            # Credito de tesoros a jugadores que se desconectaron antes del flush
            ...
            # Flags desde assetbans si el modulo existe en FH2 (opcional).
            ...
            if _IN_GAME:
                _debug_log('upload ok=%s msg=%s' % (ok, msg))
            self.round_snapshots.clear()
        except Exception as exc:
            if _IN_GAME:
                _debug_log('flush error: %s' % exc)
            else:
                print('latamstats flush error: %s' % exc)


# Arrancar al importar el modulo en el servidor FH2.
# Solo "import latamstats" no alcanza si falla _IN_GAME; con bf2 llama init().
if _IN_GAME:
    init()
